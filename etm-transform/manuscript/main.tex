...
\begin{table}[H]
    ...
    \begin{tabular}{|c|c|c|c|c|}
        ...
        \hline
        % TODO: change to ML-KEM-512/768/1024
...
\begin{figure}[H]
    \begin{floatrow}
        \ffigbox{
            \begin{algorithm}[H]
                \caption{$\texttt{UKEX}_\texttt{C}$}\label{alg:unauthenticated-kex-client}
                \begin{algorithmic}[1]
        ...
        \ffigbox{
            \begin{algorithm}[H]
                \caption{$\texttt{UKEX}_\texttt{S}$}\label{alg:unauthenticated-kex-server}
                \begin{algorithmic}[1]
...
The round trip time measures the time between the moment before client generates the ephemeral keypair and the moment after client derives the session key. A total of 10000 rounds of key exchanges were performed, with the median and average RTT recorded below:
...
\begin{figure}[H]
    \begin{floatrow}
        \ffigbox{
            \begin{algorithm}[H]
                \caption{$\texttt{UAKE}_\texttt{C}(\pk_s)$}\label{alg:uakex-client}
                \begin{algorithmic}[1]
        ...
        \ffigbox{
            \begin{algorithm}[H]
                \caption{$\texttt{UAKE}_\texttt{S}(\sk_s)$}\label{alg:uakex-server}
                \begin{algorithmic}[1]
...
\begin{figure}[H]
    \begin{floatrow}
        \ffigbox{
            \begin{algorithm}[H]
                \caption{$\texttt{AKE}_\texttt{C}(\pk_s, \sk_c)$}\label{alg:akex-client}
                \begin{algorithmic}[1]
                    \Require Server's long-term $\pk_s$
                    \Require Client's long-term $\sk_s$
                    \State $(\pk_e, \sk_e) \leftsample \keygen()$
                    \State $(c_s, K_s) \leftsample \encap(\pk_s)$
                    \State $\texttt{send}(\pk_e, c_s)$
