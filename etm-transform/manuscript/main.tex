...
\begin{table}[h]
    ...
    \begin{tabular}[t]{|p{1.45cm}|p{1.0cm}|p{1.0cm}|}
        \hline
        \multicolumn{2}{|c|}{\bf Input size: 768 bytes} \\
        \hline
        MAC & Median \newline (ccl)\\
        \hline
        Poly1305 & 909 \\
        ...
        CMAC & 6291 \\
        \hline
        KMAC \newline 128-bit tag & 6373 \\
        \hline
    \end{tabular}
    % TODO: 128-bit tag size is not suitable for higher security level.
    \begin{tabular}[t]{|p{1.45cm}|p{1.0cm}|p{1.0cm}|}
        \hline
        \multicolumn{2}{|c|}{\bf Input size: 1088 bytes} \\
        \hline
        MAC & Median \newline (ccl) \\
        % \hline
        % Poly1305 & 961 & 2704 \\
        ...
        % CMAC & 7305 & 7588 \\
        \hline
        KMAC \newline 192-bit tag & 9697 \\
        \hline
    \end{tabular}
    \begin{tabular}[t]{|p{1.45cm}|p{1.0cm}|p{1.0cm}|}
        \hline
        \multicolumn{2}{|c|}{\bf Input size: 1568 bytes} \\
        \hline
        MAC & Median \newline (ccl) \\
        % \hline
        % Poly1305 & 1065 & 1809 \\
...
In unilaterally authenticated key exchange (UAKE), the authenticating party proves its identity to the other party by demonstrating possession of a secret key that corresponds to a published long-term public key. In this implementation, the client possesses the long-term public key $\pk_S$ of the server, and the server authenticates itself by correctly decrypting the challenge encryption sent by the client. UAKE routines are summarized in Figure \ref{fig:uake-routines}. Performance data is summaried in Table \ref{tbl:uake-rtt}.
