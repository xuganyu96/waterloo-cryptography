...
\subsection{Choosing message authentication code}\label{sec:mac-performance}
Given that each keypair is only used once, an adversary can only obtain one authenticated ciphertext, which means that the corresponding MAC key is also only used once per key exchange. This allows us to use MAC constructions that are only one-time secure for substantial computation efficiency gains. We compared the median and mean number of CPU cycles (Intel i7-9750H) used to sign Kyber's IND-CPA ciphertexts. The results are listed in \ref{tbl:standalone-mac-perf}.
