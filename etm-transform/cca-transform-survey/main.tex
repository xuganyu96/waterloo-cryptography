...
\begin{figure}[H]
    ...
    \begin{minipage}{0.5\textwidth}
        ...
        \end{algorithm}
    \end{minipage}
    \caption{The \monospace{PA} game}\label{fig:pa-game}
\end{figure}

\monospace{PA} adversary wins the game if $c$ is not in $\mathcal{L}^\mathcal{E}$ and the knowledge extractor fails to recover the correct decryption.
...
\begin{theorem}[\cite{bellare1998relations} theorem 4.2]\label{thm:pa-implies-cca}
    If $\pke$ is $\epsilon$-plaintext aware, then for every \monospace{IND-CCA2} adversary $B$ against $\pke$ making at most $q_D$ decryption queries, there exists an \monospace{IND-CPA} adversary $A$ such that
